...
public class AbilityEffectMoneySO : AbilityEffectSO
{
    ...
    {
        int money = DiceEffectCalculator.GetCalculatedEffectValue(moneyAmount, context.abilityDice.DiceValue, calculateType);

        MoneyManager.Instance.Money += money;
        TriggerAnimationManager.Instance.PlayTriggerAnimation(context.abilityDice.transform, Vector3.down, money);
